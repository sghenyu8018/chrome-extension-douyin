...
| name | string | 达人名称 |
| fans | number | 粉丝数 |
| category | string | 类别（如：服饰内衣） |
| region | string | 地区（如：宁夏·中卫） |
| priceRange | string | 价格范围（如：¥2.5万-5万） |
| tags | string[] | 标签数组（如有联系方式、回复率高） |
| avatar | string | 头像个URL |
...
4. **数据导出格式**：当前支持JSON格式，可以扩展为CSV、Excel等格式
...
2. **分页限制**：当前版本只抓取当前页面可见的数据，不支持自动翻页
3. **存储限制**：localStorage有容量限制，建议定期导出和清理数据
